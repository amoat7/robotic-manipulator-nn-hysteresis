function [xdot]= statefeedback(t,x)
%% desired trajectory
...
torque=-e-(k2*e2)-(B*tanh(e2/b));
% updating laws
 dW = gam_w*( zeros(16,1)*e2' - sigma_w*W ); %for the sake of matrix multiplication zeros(16,1) was previously S. 
...
xdot=[x(4);x(5);x(6);qdotdot(1);qdotdot(2);qdotdot(3)
    ...
    (torque(3)-x(9));
    dW(:,1);dW(:,2);dW(:,3); dB(:,1);dB(:,2); dB(:,3);dphit(1);dphit(2);dphit(3)
    ];
